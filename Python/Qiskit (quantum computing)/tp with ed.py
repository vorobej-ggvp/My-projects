# Alice want to share the password to Bob. Eve want to know the password to hack them afterwards
...
qc.h(1)
qc.cx(1, 2) # Entangle Alice's and Bob's qubits in X
qc.barrier(label="Bob and Alice: entanglement")

# Alice measures her secret qc[0] qubit and her qc[1] qubit
qc.cx(0, 1) # Secret and pure qubits should interact in X
qc.h(0)
qc.barrier(label="Alice: measuremants")

# Eve eavesdropped: she measures Bob's qubit before record. It breaks Alice's and Bob's entanglement
qc.measure(2, 0) # In Z because it's standart
qc.barrier(label="Eve: eavesdropping")
...
# But after Eve measured Bob's qubit Bob receive trash, not info
